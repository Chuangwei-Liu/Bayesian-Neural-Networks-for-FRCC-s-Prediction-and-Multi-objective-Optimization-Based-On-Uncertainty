...
if len(sys.argv) > 2:
    val_col = sys.argv[2]
    print(f"Program 2: Received and set training column to {val_col}")
...
def validate_and_plot(y_col, hidden_size, lr, epochs, train_loss_history, test_size=0.2, dropout_p=0.1):
    ...
    plot_loss(train_loss_history, y_col, f"{graph_path}/loss_curve.png")
...
if __name__ == "__main__":
    all_results = {}
    optimal_params_df = pd.read_excel(f"{EXCEL_PATH}/{val_col}/{val_col}_{seed}_excel/{seed}_{val_col}_optimal_params.xlsx", index_col=0)
    train_loss_histories = pd.read_excel(f"{EXCEL_PATH}/{val_col}/{val_col}_{seed}_excel/{seed}_train_loss_histories.xlsx", index_col=0)[val_col].dropna().tolist()
    results = validate_and_plot(
        val_col,
        epochs = 2000,
        hidden_size = optimal_params_df.loc[val_col, 'hidden_size'],
        lr = optimal_params_df.loc[val_col, 'lr'],
        dropout_p= optimal_params_df.loc[val_col, 'dropout_p'],
